...
from dankpy import utilities, color, dt, dankframe, document, audio, document
...
for i, t in targets.iterrows():
    ...
    fig.savefig(rf"projects/Dissertation/proposal/figures/bug_bytes/{t['sensor']} - {t['target']}_spectrogram.pdf", dpi=300)
    ...
    ax.hlines([rms], 0, round(a.data["time [s]"].max()), color="red", linestyle="dashed", label="NSPA ({:.2f} dB)".format(20*np.log10(rms)))
    # plot rectange at cutoffs 
    ax.fill_between(a.data["time [s]"], min_threshold, max_threshold, color="red", alpha=0.25, label="Analysis Region")
    ax.legend(ncols=2, loc="upper right")

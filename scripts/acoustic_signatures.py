...
#%%
record = db.session.get(spidb.Record, targets[0]["record"])
...
fig.savefig("projects/Dissertation/proposal/figures/acoustic_signatures/{}_{}_spectrogram_display.pdf".format(targets[0]["target"], targets[0]["material"]), dpi=300)
...
for i, t in enumerate([targets[0], targets[2], targets[3]]):
    ...
    ax.plot(f, p, label=f"{lookup.lookup(key=t['target'], latex=True, min=True)} ({t['material']})", color=color.colors[i])
ax.plot(noise["frequency"], noise["average_db"], color="black", linestyle="solid", label="Reference", zorder=10)
ax.set_xlim(0, 8000)
ax.set_ylim(-125, -25)
ax.set_yticks([-125, -75, -25])
...
for i, t in enumerate(targets):
    ...
    ax.plot(f, snr, label=f"{lookup.lookup(key=t['target'], latex=True, min=True)} ({t['material']})", color=color.colors[i])
ax.set_xlim(0, 8000)
ax.set_ylim(0, 50)
ax.set_yticks([0, 25, 50])
ax.set_xlabel("Frequency [Hz]")
ax.set_ylabel("SNR [dB]")
ax.legend(bbox_to_anchor=(0., 1.10, 1, 0), loc="lower center", ncol=2, columnspacing=1, markerscale=0.5, fontsize=8)
...
for i, t in enumerate(targets):
    ...
    ax.set_ylabel("Amplitude")
    ax.legend(loc="upper right", handlelength=0, handletextpad=0)
    fig.savefig("projects/Dissertation/proposal/figures/acoustic_signatures/{}_{}_waveform_envelope.pdf".format(t["target"], t["material"]), dpi=300)

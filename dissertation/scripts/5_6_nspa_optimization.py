...
data = pd.read_csv(r"projects/MDPI-Detection/data/nspa_grid.csv")
# %%
...
nspa_low_freq = pd.read_csv("projects/MDPI-Detection/data/nspa_grid_specific.csv")
average = nspa_low_freq.groupby("frequency").mean().reset_index()

fig, ax = plt.subplots()
ax.scatter(
    values[values.low < 1400]["low"], values[values.low < 1400]["nspa"], c="black", s=5
)
ax.scatter(
...
ax.scatter(
    convergence["frequency"], convergence["nspa_diff"], color="red", s=5, zorder=3
)

average = average[average["frequency"] < 1550]  # remove the outlier at 1500
ax.scatter(
    average["frequency"].iloc[::5], average["nspa_diff"].iloc[::5], c="black", s=5
)

ax.set_xlim(1000, 5000)
ax.set_ylim(22, 42)
ax.set_yticks([22, 32, 42])
ax.set_xlabel("Lower Frequency ($\\omega_l$) [Hz]")
ax.set_ylabel("NSPA Difference [dB]", fontsize=8)
fig.savefig(

...
fig, ax = plt.subplots()
im = ax.imshow(grid, aspect="auto", origin="lower", extent=[low_freq.min(), low_freq.max(), high_freq.min(), high_freq.max()], cmap="jet")
cbar = fig.colorbar(im, ax=ax, label="NSPA [dB]")
cbar.set_label(label='NSPA [dB]', size=9)
# cbar.set_ticks([44, 48])
ax.set_xlim(low_freq.min(), low_freq.max())
# ax.set_xticks([4500, 5250, 6000])
ax.set_ylim(high_freq.min(), high_freq.max())
...
fig, ax = plt.subplots()
im = ax.imshow(grid, aspect="auto", origin="lower", extent=[low_freq.min(), low_freq.max(), high_freq.min(), high_freq.max()], vmin=0, vmax=40,cmap="jet")
cbar = fig.colorbar(im, ax=ax)
cbar.set_label(label='NSPA Difference [dB]', size=7)
cbar.set_ticks([0, 20, 40])
ax.set_xlim(low_freq.min(), low_freq.max())
ax.set_ylim(high_freq.min(), high_freq.max())
ax.set_yticks([3000, 5500, 8000])
...
#%%
fig, ax = plt.subplots()
ax.scatter(values["low"], values["nspa"], c="black", s=5)

#plot every 10th point of average
# ax.scatter(average["frequency"], average["nspa_diff"], c="black", s=2)
ax.scatter(average["frequency"].iloc[::5], average["nspa_diff"].iloc[::5], c="black", s=2)
...
ax.scatter(convergence["frequency"], convergence["nspa_diff"], color="red", s=5, zorder=3)
...
ax.set_ylabel("NSPA Difference [dB]", fontsize=5)

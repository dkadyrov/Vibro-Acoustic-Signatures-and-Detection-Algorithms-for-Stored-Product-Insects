...
\usepackage{caption}
\captionsetup{font=footnotesize, margin=30pt, skip=5pt} % Set caption font to 8pt with smaller margins
...
\usepackage{showframe} % Shows page layout boxes - REMOVE when done
...
\newcommand{\committeenameA}{Dr. Raju Datla, Chairman}
 ...
 {A DISSERTATION}\\
  ...
  % for master thesis, change Chairman to Advisor
    ...
    \begin{spacing}{ }\noindent \rule{3.2in}{0.1mm}
        ...
        % for master thesis, change Chairman to Advisor
        {\thesischairadvisor}, Chair  \hfill{Date}\\[2mm]
        {\noindent \rule{3.2in}{0.1mm}}\\[-1.3mm]
...
Stored products, such as grains and processed foods, are a staple in global diets and one of the most internationally traded commodities but are susceptible to infestation by various pests, most notably insects. The early detection of insects throughout moments of the supply chain is crucial to mitigate against qualitative and quantitative losses, ensure food safety, and prevent introducing invasive species to new environments resulting in disproportionate ecological harm. 
...
A normalization method was implemented using the ambient noise of the sensors as a reference to accommodate for proprietary, non-calibrated sensors and enable relative detection thresholds for unknown sensitives. The normalized envelope of the filtered signals was used to characterize and compare the insect signals by measuring the Normalized Signal Pulse Amplitude (NSPA) and Normalized Signal Energy Level (NSEL), parameters that apply the signal noise ratio (SNR) for pulse-based detection and averaged energy-based detection, respectively. The NSPA metric demonstrated an effective method for a threshold-based detection algorithm.

Although the physical insulation in the platform provides up to 50 dB of noise attenuation, external noise still penetrates to the internal piezoelectric sensors appearing as insect-like signals that could lead to false positives. The NSPA algorithm was parametrized to determine a filter range that would limit the effects of external noise up to 80 dB. External microphones were also included in the design to identify high peak-to-RMS signals outside the sensor and eliminate them from the internal piezoelectric sensor waveforms. This leads to a \SI{100}{\percent} detection rate for the available recordings. 
...
I would like to thank my other committee members. Dr. Raju Datla for serving as the committee chair and guiding me throughout the Ph.D. process. Dr. Muhammad Hajj for his feedback that elevated the qualtify of my research. Dr. Richard Mankin for serving as an external member and the lengthy, extensive, and high quality research he has conducted in the field of insect detection that served as a foundation for this work. 

I would have never started this journey if not for the initial motivation of my love, Gabriela Sanchez, and her unwavering support. 

Unquestionable support came from my former coworkers at the Sensor Technology and Applied Research (STAR) Center laboratory and Maritime Security Center (MSC) at Stevens Institute of Technology including Dr. Hady Salloum, Dr. Alexander Sedunov, Nikolay Sedunov, Christopher ``Kip'' Francis, Sergey T., Aleksandr M., Cameron Ruddy, Anthony Rusignioli, Beth DeFares, and Dr. Barry Bunin. Besides providing support with the development and deployment of systems and sensors, expanding my Russian lexicon to the dismay of my mother, and improving my chess game they all contributed to my growth as a researcher and engineering, instiling lifelong skills and passions as well as a strong importance and appreciation of engineering to national and global wellbeing. 

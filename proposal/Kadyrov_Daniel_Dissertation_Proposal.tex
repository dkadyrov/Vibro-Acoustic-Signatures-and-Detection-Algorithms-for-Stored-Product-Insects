...
\usepackage{todonotes} % Add this line
...
\renewcommand{\maketitle}{
  \begin{center}
    ...
    \vskip 1em
	{\large A DISSERTATION PROPOSAL \par}
    \vskip 1.5em
	{Submitted to the Faculty of Stevens Institute of Technology\\in partial fulfillment of the requirements for the degree of \par} 
	\vskip 1.5em
	{\large DOCTOR OF PHILOSOPHY \par}
    % {\large \@date \par}
	\vskip 1.5em
	Daniel Kadyrov, Candidate\\
	\vskip 1.5em
	ADVISORY COMMITTEE\\
	\vskip 1em
	Alexander Sutin\\
...
Stored Product Pests (SPP) refers to the insects that target products such as grains, legumes, spices, and processed foods throughout supply chain. An undetected or unmitigated insect infestation can lead to quantitative and qualitative damage to the commodity as they consume and contaminate the material, promote infections, and accelerate deterioration. Furthermore, the introduction of non-native species into ecological environments can exacerbate negative effects. As a result, efficaciously inspecting products for pests throughout the global supply chain is not only crucial for the integrity of the product and mitigate greater infestation but also secure regional agricultural and environmental ecologies. 

A strategic gateway to control the entry of SPPs into the United States is through inspection at Ports of Entry (PoE) such as seaports, airports, and land border crossings. Currently, the main method of stored product inspection at these locations depends on manual visual examination by Customs and Border Protection Agricultural Specialists. This process is labor-intensive, time-consuming, and prone to human error, making it less effective in detecting pests that may be hidden within the product. Various physical, chemical, spectral, and microwave methods have been explored to enhance and automate detection capabilities but these technologies are often relatively expensive, complex, require specialized training, extensive time, or may damage the produce in the process.  

Passive acoustic sensing has been a reliable and relatively low-cost alternative to even the most advanced sensing technologies to provide a method of detecting, localizing, tracking, and classifying subjects including but not limited to wood boring insects \cite{flynnAcousticMethodsInvasive2016, sutinVibroacousticMethodsInsect2017, sutinAutomatedAcousticDetection2019}, marine mammals, scuba divers and surface vessels \cite{sutinStevensPassiveAcoustic2010a, sutinStevensPassiveAcoustic2013, pollaraPassiveAcousticMethods2017, sedunovLowSizeCostAcoustic2022, salloumLowcostMultimodalIntegrated2022, sedunovStevensPassiveAcoustic2022a}, low-flying aircrafts \cite{sutinAcousticDetectionTracking2013, sedunovPassiveAcousticLocalization2013}, and unmanned aerial vehicles \cite{yakubovskiyFeatureExtractionAcoustic2015, salloumAcousticSystemLow2015, sedunovUAVPassiveAcoustic2018, sedunovStevensDroneDetection2019}. The presentation and subsequent conference paper at the Acoustic Society of America Conference in 2022 on the topic of \citetitle{kadyrovImprovementsStevensDrone2022} showed promising results on the capabilities of acoustic sensors to detect various targets using signal-processing techniques \cite{kadyrovImprovementsStevensDrone2022}. Within stored product insect detection, acoustic detection is significantly lower priced than other methods and can detect insects hidden within kernels. It can also provide real-time monitoring which can be performed non-invasively and without product loss or damage.
...
The Acoustic Stored Product Insect Detection System (A-SPIDS) was developed to address the need for an efficient alternative to manual visual inspection of stored products for insects. Originally funded by the Department of Homeland Security (DHS) Cross-Border Threat Screening and Supply Chain Defense (CBTS) to research and prototype alternative low-cost sensors, expanded into deeper analysis utilizing the recordings made of live insect specimen in various materials using the systems to study their acoustic characteristics, explore normalization methods, investigate reducing the influence of background noise, and implement a detection algorithm. 

A-SPIDS uses piezoelectric disc sensors to capture the acoustic signals generated by insects as they move around the product. The system was used to collect an extensive amount of recordings using live insect specimens at various life stages in materials of different kernel size and hardness while exposed to varying levels of natural and simulated background noise. A database package was created to assist in collecting, storing, organizing, and retrieving the recorded audio and its associated metadata. The Stored Product Insect Database (SPIDB) extends the Sound Organization for Network Integration, and Collection/Collaboration (SONICDB) framework that was used in other acoustic projects including the Boat Detection System \cite{salloumLowcostMultimodalIntegrated2022} and the Drone Acoustic Database (DADB) \cite{danielkadyrovDkadyrovSpidbSPIDB2024}. The data was published as the Acoustic Stored Product Insect Dataset on Kaggle \cite{danielkadyrovStoredProductInsect}. 

In \citetitle{kadyrovVibroAcousticSignaturesVarious2024}, the impulse signals generated by the insects as they manuevered around the material were analyzed to identify unique spectral and temporal characteristics. Since the piezoelectric discs are not calibrated, a method of normalization was introduced that used the system's self-noise as a reference level to compare the respective signals. When self-noise was not available, a normalization method using the median of the signal was shown to be just as effective. 
...
One challenge in acoustic sensing is the interference of background noise influencing the detection of target signals. Although A-SPIDS was built with significant physical insulation that was shown to reduce noise by an average of \SI{35}{\deci\bel}, the sensitive piezoelectric sensors capture external noise which often manifested as signals similar to those produced by insects and would result as false positives. Microphones were included in the system design to provide situational awareness and minimize its impact. In the upcoming work, we will focus on investigating how to estimate the levels of background noise using the external microphone by correcting its readings with recordings made in when a calibrated SPL meter was available. The original implementation of the NSPA and NSEL metrics could not overcome the influence of external noise but through spectral analysis of the insect signals when significant background is present, a frequency bandwidth will be selected where the target signals could be effectively isolated when exposed to noise levels of up to \SI{90}{\deci\bel A}. 
...
% \newpage
% \subsubsection{Optimizing Frequency Range to Limit External Noise Influence}
...
% \todo[inline]{This subsection will describe the design of the detection algorithm.}

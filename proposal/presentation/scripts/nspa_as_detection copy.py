...
# find where pfn and pfp intercept


fig, ax = plt.subplots()
# ax.plot(results.threshold, results.pod, label="POD", color=color.colors[0], linestyle="--")
# ax.plot(results.threshold, results.pfp, label="PFP", color=color.colors[1], linestyle="--")
# ax.plot(results.threshold, results.pfn, label="PFN", color=color.colors[2], linestyle="--")

# smooth the pfp line using a savgol filter
from scipy import signal


results["pod_smooth"] = signal.savgol_filter(results.pod, 11, 1, mode="nearest")
results["pod_smooth"] = np.clip(results["pod_smooth"], 0, 100)
ax.plot(results.threshold, results.pod_smooth, label= "POD", color=color.colors[0])

results["pfp_smooth"] = signal.savgol_filter(results.pfp, 11, 1, mode="nearest")
results["pfp_smooth"] = np.clip(results["pfp_smooth"], 0, 100)
ax.plot(results.threshold, results.pfp_smooth, label= "PFP", color=color.colors[1])

results["pfn_smooth"] = signal.savgol_filter(results.pfn, 11, 1, mode="nearest")
results["pfn_smooth"] = np.clip(results["pfn_smooth"], 0, 100)
ax.plot(results.threshold, results.pfn_smooth, label= "PFN", color=color.colors[2])

best = results.iloc[(results["pfp_smooth"] - results["pfn_smooth"]).abs().idxmin()]

ax.axvline(
    best.threshold,
    color="gray",
    linestyle="--",
    label=f"Best Threshold: {best.threshold:.1f} dB NSPA\nPOD: {best.pod_smooth:.1f}%, PFP: {best.pfp_smooth:.1f}%, PFN: {best.pfn_smooth:.1f}%",
)

ax.set_xlabel("Detection Threshold NPSA [dB]]")

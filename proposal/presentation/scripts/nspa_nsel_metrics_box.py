...
samples = db.session.query(spidb.Sample).filter(spidb.Sample.sensor_id == 1).filter(spidb.Sample.channel_id.in_([1,2,3,4])).filter(spidb.Sample.subject_id.in_([1,2,3,4])).all()
...
# #%%
sub = s[["record_id", "subject_id", "material_id", "nspa 1565-6000", "nspa 500-6000", "spl 500-6000", "spl 1565-6000"]].groupby(["record_id"]).max().reset_index()
...
for i, row in enumerate(external_noise[f"{band}"]):
    ...
    ax.text(20.25, row, f"{noise_lvl[i]} dB", fontsize=8, color="black", zorder=10, va="center")
